package com.example.storagetestingapp.repo

import android.graphics.Bitmap
import android.graphics.BitmapFactory
import android.util.Log
import com.example.storagetestingapp.app.MyApplication
import java.io.*
...
object FileRepository {
    ...
    fun readTextFile(fileName: String): String? {
        return try {
            ...
            Log.d(
                TAG,
                exception.message ?: "$fileName not found in directory ${getStorageLocation()}"
            )
            null
        } catch (exception: IOException) {
            Log.d(TAG, exception.message ?: "Error saving file $fileName")
